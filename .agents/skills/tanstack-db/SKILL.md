...
const todoCollection = createCollection(
  queryCollectionOptions({
    ...
    onInsert: async ({ transaction }) => {
      await Promise.all(transaction.mutations.map((mutation) => api.todos.create(mutation.modified)));
    },
    onUpdate: async ({ transaction }) => {
      await Promise.all(transaction.mutations.map((mutation) => api.todos.update(mutation.modified)));
    },
    onDelete: async ({ transaction }) => {
      await Promise.all(transaction.mutations.map((mutation) => api.todos.delete(mutation.original.id)));
    },
  }),
);
```
...
const collaborativeCollection = createCollection(queryCollectionOptions({ syncMode: "progressive" /* ... */ }));
```
